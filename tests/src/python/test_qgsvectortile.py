...
class TestVectorTile(QgisTestCase):
    ...
    def testEncodeDecodeUri(self):
        ...
        self.assertEqual(
            ...
            {
                "type": "xyz",
                "url": "https://fake.server/{x}/{y}/{z}.png",
                "zmin": "0",
                "zmax": "2",
            },
        )
        ...
        self.assertEqual(
            parts,
            {
                ...
                "url": "https://fake.server/{x}/{y}/{z}.png",
                "zmax": "2",
                "http-header:referer": "https://qgis.org/",
                "referer": "https://qgis.org/",
                "styleUrl": "https://qgis.org/",
            },
        )
    ...
    def testSelection(self):
        layer = QgsVectorTileLayer(
            f"type=vtpk&url={unitTestDataPath() + '/testvtpk.vtpk'}", "tiles"
        )
        ...
        self.assertCountEqual(
            {f.geometry().asWkt(-3) for f in layer.selectedFeatures()},
            {
        ...
        self.assertCountEqual(
            ...
            ["polys", "polys", "polys", "lines", "points", "polys", "polys", "polys"],
        )
        ...
        self.assertCountEqual(
            {f.id() for f in layer.selectedFeatures()}, {2220498092032, 3320043274243}
        )
        self.assertCountEqual(
            {f.geometry().asWkt(-3) for f in layer.selectedFeatures()},
            {
        ...
        self.assertCountEqual(
            {f.id() for f in layer.selectedFeatures()}, {2220498092032}
        )
        self.assertCountEqual(
            {f.geometry().asWkt(-3) for f in layer.selectedFeatures()},
            {"Point (-12714000 5220000)"},
        ...
        self.assertCountEqual({f.id() for f in layer.selectedFeatures()}, {33554436})
