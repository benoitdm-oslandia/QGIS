  ...
  : Qgs3DEditionToolBar( u"Point cloud edition"_s, parent )
{
  ...
  QAction *actionPointCloudChangeAttributeTool = mEditingToolsMenu->addAction( QIcon( QgsApplication::iconPath( u"mActionSelectPolygon.svg"_s ) ), tr( "Select by Polygon" ), this, &Qgs3DPointCloudEditionToolBar::changePointCloudAttributeByPolygon );
  QAction *actionPaintbrush = mEditingToolsMenu->addAction( QIcon( QgsApplication::iconPath( u"propertyicons/rendering.svg"_s ) ), tr( "Select by Paintbrush" ), this, &Qgs3DPointCloudEditionToolBar::changePointCloudAttributeByPaintbrush );
  QAction *actionAboveLineTool = mEditingToolsMenu->addAction( QIcon( QgsApplication::iconPath( u"mActionSelectAboveLine.svg"_s ) ), tr( "Select Above Line" ), this, &Qgs3DPointCloudEditionToolBar::changePointCloudAttributeByAboveLine );
  QAction *actionBelowLineTool = mEditingToolsMenu->addAction( QIcon( QgsApplication::iconPath( u"mActionSelectBelowLine.svg"_s ) ), tr( "Select Below Line" ), this, &Qgs3DPointCloudEditionToolBar::changePointCloudAttributeByBelowLine );
...
bool Qgs3DPointCloudEditionToolBar::accept( QgsMapLayer *layer ) const
{
  if (layer != nullptr && layer->type() == Qgis::LayerType::PointCloud)
  {
    ...
    for ( QgsPointCloudSubIndex &subIndex : pcLayer->subIndexes() )
    {
      ...
      {
        QgisApp::instance()->messageBar()->pushMessage( tr( "Virtual Point Cloud editing" ), tr( "Some of the files referenced by the VPC have not yet been loaded, selection of areas of a not yet loaded dataset will not cause any changes to its data. Only actually selected points will be edited." ) );
        break;
      }
    }
    return true;
