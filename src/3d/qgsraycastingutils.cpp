...
namespace QgsRayCastingUtils
{
  ...
  float rayTriangleMinAngle( const QgsRay3D &ray, const QList<QVector3D> &pointList, QVector3D &minPoint )
  {
    float minAngle = 999999;
    for ( const QVector3D i : pointList )
    {
      float angle = ray.angleToPoint( i );
      if ( angle < minAngle )
      {
        minAngle = angle;
        minPoint = i;
