...
QgsPostprocessingRenderView::QgsPostprocessingRenderView( QObject *parent
    , QgsShadowRenderView *shadowRenderView
    , QgsForwardRenderView *forwardRenderView
    , QgsAbstractRenderView *aoRenderView
    , QSize mSize
    , Qt3DCore::QEntity *rootSceneEntity )
  : QgsAbstractRenderView( parent, "post_processing" )
{
...
void QgsPostprocessingRenderView::updateTargetOutputSize( int width, int height )
{
  QgsAbstractRenderView::updateTargetOutputSize( width, height );
...
{
  if ( ! isSubTreeEnabled() || mRenderCaptureTargetSelector->isEnabled() == enabled )
    return;

  mRenderCaptureTargetSelector->setEnabled( enabled );
}

void QgsPostprocessingRenderView::setupEyeDomeLighting( bool enabled, double strength, int distance )
{
...
void QgsPostprocessingRenderView::setAmbientOcclusionEnabled( bool enabled )
{
