...
namespace
{
  ...
  QString formatNode( const Qt3DCore::QNode *n )
  {
    QString res = QLatin1String( "(%1%2)" )
                  ...
                  .arg( formatIdName( n->id().id(), n->objectName() ) );
    if ( !n->isEnabled() )
      res += QLatin1String( " [D]" );
    return res;
  }

  QString formatList( const QStringList &lst )
  {
    return QString( QLatin1String( "[ %1 ]" ) ).arg( lst.join( QLatin1String( ", " ) ) );
  }

  QString formatLongList( const QStringList &lst, int level )
  {
    ...
    {
      l = QString( "-> %1\n" ).arg( l );
      out += l.rightJustified( l.length() + ( 1 + level ) * 2, ' ' );
    }
    QString end( QLatin1String( "]" ) );
    return out + end.rightJustified( end.length() + ( 1 + level ) * 2, ' ' );
  }

  QString formatField( const QString &name, const QString &value )
  {
    if ( value == "<no_value>" )
  ...
  QString dumpSGEntity( const Qt3DCore::QEntity *n, int level )
  {
    auto extractTextureParam = []( const QVector<Qt3DRender::QParameter *> &params, QStringList & fl )
    {
    ...
    if ( components.size() )
    {
      ...
      {
        QString res = formatNode( c );
        QStringList fl;
        if ( qobject_cast<const Qt3DExtras::QTextureMaterial *>( c ) )
        {
          const auto *c_cast = qobject_cast<const Qt3DExtras::QTextureMaterial *>( c );
          if ( c_cast->texture() )
          {
            const auto texImages = c_cast->texture()->textureImages();
            for ( const auto *texImg : texImages )
            {
              fl += formatField( texImg->metaObject()->className(), formatIdName( texImg->id().id(), texImg->objectName() ) );
            }
          }
        }
        if ( qobject_cast<const Qt3DRender::QMaterial *>( c ) )
        {
          const auto *c_cast = qobject_cast<const Qt3DRender::QMaterial *>( c );
          if ( c_cast->effect() )
          {
            const auto techniques = c_cast->effect()->techniques();
            for ( const auto *tech : techniques )
            {
              extractTextureParam( tech->parameters(), fl );
              const auto passes = tech->renderPasses();
              for ( const auto *pass : passes )
              {
                extractTextureParam( pass->parameters(), fl );
              }
            }
            extractTextureParam( c_cast->effect()->parameters(), fl );
          }
          extractTextureParam( c_cast->parameters(), fl );
          if ( !fl.empty() )
            res += formatList( fl );
        }
  ...
  QStringList dumpSG( const Qt3DCore::QNode *n, int level = 0 )
  {
    ...
    if ( entity != nullptr )
    {
      QString res = dumpSGEntity( entity, level );
  ...
  QString dumpFGNode( const Qt3DRender::QFrameGraphNode *n )
  {
    QString res = formatNode( n );

    if ( qobject_cast<const Qt3DRender::QLayerFilter *>( n ) )
    {
      ...
      for ( auto l : layers )
      {
        sl += formatIdName( l->id().id(), l->objectName() );
      }
    ...
    else if ( qobject_cast<const Qt3DRender::QCameraSelector *>( n ) )
    {
      const auto *cs = qobject_cast<const Qt3DRender::QCameraSelector *>( n );
      QStringList fl;
      fl +=  formatField( cs->camera()->metaObject()->className(),
                          formatIdName( cs->camera()->id().id(), cs->camera()->objectName() ) );
      res += QString( " %1" ).arg( formatList( fl ) );
    }

    else if ( qobject_cast<const Qt3DRender::QRenderStateSet *>( n ) )
    {
      const auto *rss = qobject_cast<const Qt3DRender::QRenderStateSet *>( n );
      QStringList sl;
      const auto renderStates = rss->renderStates();
      for ( auto rs : renderStates )
      {
        if ( qobject_cast<const Qt3DRender::QCullFace *>( rs ) )
        {
        ...
        else if ( qobject_cast<const Qt3DRender::QBlendEquation *>( rs ) )
        {
        ...
        else if ( qobject_cast<const Qt3DRender::QBlendEquationArguments *>( rs ) )
        {
        ...
        else if ( qobject_cast<const Qt3DRender::QColorMask *>( rs ) )
        {
        ...
        else if ( qobject_cast<const Qt3DRender::QDepthTest *>( rs ) )
        {
        ...
        else if ( qobject_cast<const Qt3DRender::QPointSize *>( rs ) )
        {
        ...
        else if ( qobject_cast<const Qt3DRender::QPolygonOffset *>( rs ) )
        {
    ...
    else if ( qobject_cast<const Qt3DRender::QRenderTargetSelector *>( n ) )
    {
      const auto *rs = qobject_cast<const Qt3DRender::QRenderTargetSelector *>( n );
      if ( rs->target() )
      {
        QStringList sl;
...
Qt3DRender::QFrameGraphNode *QgsFrameGraph::constructPostprocessingPass()
{
  ...
  constructSubPostPassForTexturesPreview()->setParent( top );
  constructSubPostPassForRenderCapture()->setParent( top );
...
QString QgsFrameGraph::dumpFrameGraph() const
{
  ...
  while ( top->parent() && dynamic_cast<Qt3DRender::QFrameGraphNode *>( top->parent() ) )
    top = top->parent();
  QStringList strList = dumpFG( dynamic_cast<Qt3DRender::QFrameGraphNode *>( top ) );
  return qPrintable( strList.join( "\n" ) ) + QString( "\n" );
}

QString QgsFrameGraph::dumpSceneGraph() const
{
  QStringList strList = dumpSG( mRootEntity );
  return qPrintable( strList.join( "\n" ) ) + QString( "\n" );
}
